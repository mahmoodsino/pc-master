...
import { handelItemGuestToUser, TokenAtom, YouHaveItemsModalAtom } from '../../../../helper'
    ...
    <>
      ...
      >
        <div className=" sm:px-5 md:px-16 py-10">
          <h1 className='text-red-950 font-bold text-lg'>you have items in your cart you want to keep ?</h1>
          <div className='flex justify-between mt-3'>
                <BaseButton onClick={() => typeof(guest_user_id) ==="number" && handelItemsGuestTouser(guest_user_id)} className='px-5 py-2 text-green-950 border border-green-950' title='yes' />
                <BaseButton onClick={() =>(setYouHaveItemsModal(false),push("./")) } className='px-5 py-2 text-red-950 border border-red-950' title='No' />
          </div>
        </div>
      </div>
